...
class AuthController extends Controller 
{
	...
	public $add_hr_min1 = "+5 hour +30 minutes"; //for andoid subcription time
	...
	public function getWallpapersAtHomeForAndroidUsers(Request $request){ #die('---');
		...
		if($deviceType != ""){
			...
			//code added at 8Feb2022
			if( isset($input["refresh_token"]) && $input["refresh_token"] !="" ){
				//update in main table
				DB::table('appusers')
				->where('userEmail',$input['userEmail'])
				->update(['android_first_refresh_token' =>$input['refresh_token'],'updated_at'=> now()]);
				$finalArr['refresh_token'] = $input["refresh_token"];
			} 
			...
			if( isset($input["access_token"]) && $input["access_token"] !="" ){
				...
				DB::table('appusers')
				->where('userEmail',$input['userEmail'])
				->update(['android_final_access_token' =>$input['access_token'],'updated_at'=> now()]);
				$finalArr['access_token'] = $input["access_token"];
			} 
			
			if( isset($input["currentimeinmillis"]) && $input["currentimeinmillis"] !="" ){
				//update in main table
				DB::table('appusers')
				->where('userEmail',$input['userEmail'])
				->update(['subscriptionStartDate' =>$input["currentimeinmillis"],'updated_at'=> now()]);
				$finalArr['currentimeinmillis'] = $input["currentimeinmillis"];
			} 
			
			if( isset($input["expiryTimeMillis"]) && $input["expiryTimeMillis"] !="" ){
				//update in main table
				DB::table('appusers')
				->where('userEmail',$input['userEmail'])
				->update(['subscriptionEndDate' =>$input["expiryTimeMillis"],'updated_at'=> now()]);
				$finalArr['expiryTimeMillis'] = $input["expiryTimeMillis"];
			}
			
			if( (isset($input["expiryTimeMillis"]) && $input["expiryTimeMillis"] !="")  && (isset($input["currentimeinmillis"]) && $input["currentimeinmillis"] !="") ){
				$dateDiff = $this->dateDiff($input["currentimeinmillis"], $input["expiryTimeMillis"]);
				if(!empty($dateDiff)){
					$dateDiff = $dateDiff;
				} else {
					$dateDiff = 0;
				...
				->where('userEmail',$input['userEmail'])
				->update([
				'userSubscriptionType' => $userSubscriptionType11, //Monthly
				'userSubscriptionDays' => $dateDiff,//$input["userSubscriptionDays"],
				'updated_at' => now()
				]);
			}
	...
	public function addUpdSubscriptionForAndroidUsers(Request $request){ #die('---');
		...
		if(!empty($isUsrSubscribed)){ //update user subscription as monthly user
			...
			$dateDiff = $this->dateDiff($input["subscriptionStartDate"], $input["subscriptionEndDate"]);
			...
			//echo $isUsrSubscribed->subscriptionId;
			//echo "<br/>";
			//echo $input["subscriptionId"];
			...
			'userSubscriptionDays' => $dateDiff, //7=>no of days //0 =>For PN Testing purpose
