...
class AuthController extends Controller 
{
	...
	public $trial_days = "+7 days"; //7=>for live
	public $after_trial_days = "+30 days"; //30=>for live
	
	//public $trial_days = "+7 minutes"; //7=>for test
	//public $after_trial_days = "+30 minutes"; //30=>for test
	...
	public function getWallpapersAtHomeForAndroidUsers(Request $request){ #die('---');
		...
		if($deviceType != ""){
			...
			//check user exist get their subscription info
			$isUsrExist1 = DB::table('appusers as ap')
			->select('ap.id','ap.subscriptionStartDate','ap.userSubscriptionType','ap.userSubscriptionDays','ap.subscriptionEndDate')
			->where('ap.userEmail', '=', $input["userEmail"])
			->first();
			#echo "<pre>isUsrExist1==";print_r($isUsrExist1);die;
			if(!empty($isUsrExist1)){
				if( isset($input["current_date"]) && $input["current_date"] !="" ){
					$current_dateStr = strtotime($input["current_date"]);
					$subscriptionEndDateStr = strtotime($isUsrExist1->subscriptionEndDate);
					
					//that current_date (from mobile side) > subscription end date then add 30 days for next cycle
					if($current_dateStr > $subscriptionEndDateStr){
						
						//make new expiry date for next cycle
						$nextCycleSubscriptionStartDate = $isUsrExist1->subscriptionEndDate;
						$nextCycleSubscriptionStartDateStr = strtotime($nextCycleSubscriptionStartDate);
						
						$nextCycleSubscriptionEndDateStr = strtotime($this->after_trial_days, $nextCycleSubscriptionStartDateStr);
						$nextCycleSubscriptionEndDate =  date('Y-m-d H:i:s', $nextCycleSubscriptionEndDateStr);
						
						
						
						//calculate date difference
						$dateDiff = $this->dateDiff($nextCycleSubscriptionStartDate,$nextCycleSubscriptionEndDate);
						if(!empty($dateDiff)){
							$dateDiff = $dateDiff;
						} else {
							$dateDiff = 0;
						...
						$subsArr = $this->getUsrSubscriptionType($dateDiff);
						#echo "<pre>subsArr=";print_r($subsArr);die;
						if(!empty($subsArr)){
							$userSubscriptionType11 = $subsArr['userSubscriptionType'];
							$userSubscriptionText11 = $subsArr['userSubscriptionText'];
						} else {
							$userSubscriptionType11 = "";
							$userSubscriptionText11 = "";
						}
						
						
						#echo $nextCycleSubscriptionStartDate."===".$nextCycleSubscriptionEndDate."===".$dateDiff."===".$userSubscriptionType11;die;
						//update in main table
						DB::table('appusers')
						->where('userEmail',$input['userEmail'])
						->update([
							'subscriptionStartDate' =>$nextCycleSubscriptionStartDate,
							'userSubscriptionType' =>$userSubscriptionType11,
							'userSubscriptionDays' =>$dateDiff,
							'subscriptionEndDate' =>$nextCycleSubscriptionEndDate,
							'monthly_notification_sent'=>0,
							'updated_at'=> now()
						]);
						
						$finalArr['subscriptionDate'] = $nextCycleSubscriptionStartDate;
						$finalArr['userSubscriptionType'] = $userSubscriptionType11;
						$finalArr['userSubscriptionDays'] = $dateDiff;
						$finalArr['subscriptionEndDate'] = $nextCycleSubscriptionEndDate;
					} 
				}
			} 
	...
	public function getUsrSubscriptionType($userSubscriptionDays){
		...
		} else if($userSubscriptionDays == 0){ //0 days
			$subsArr['userSubscriptionType'] = 0;
			$subsArr['userSubscriptionText'] = "0 day";
		}
		return $subsArr;
	}
	...
	public function addUpdSubscriptionForAndroidUsers(Request $request){ #die('---');
		...
		if(!empty($isUsrSubscribed)){ //update user subscription as monthly user
			...
			$subscriptionEndDateStr = strtotime($this->after_trial_days, $subscriptionStartDate);
			$subscriptionEndDate =  date('Y-m-d H:i:s', $subscriptionEndDateStr);
			
			//calculate date difference
			$dateDiff = $this->dateDiff($input["subscriptionStartDate"],$subscriptionEndDate);
			if(!empty($dateDiff)){
				$dateDiff = $dateDiff;
			} else {
				$dateDiff = 0;
			...
			$input["userSubscriptionDays"] = $dateDiff;
			...
			$subscriptionStartDate = strtotime($input["subscriptionStartDate"]);
			$subscriptionEndDateStr = strtotime($this->trial_days, $subscriptionStartDate);
			$subscriptionEndDate =  date('Y-m-d H:i:s', $subscriptionEndDateStr);
			...
			'userSubscriptionDays' => $dateDiff, //no of days
			'subscriptionEndDate' => $subscriptionEndDate,
