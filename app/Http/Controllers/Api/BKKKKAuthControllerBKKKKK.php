	...
	public function getWallpapersAtHomeForAndroidUsers(Request $request){ #die('---');
		...
		if($deviceType != ""){
			...
			if(!empty($wallpaperExist)){
				...
				->select('ap.id','ap.subscriptionStartDate','ap.userSubscriptionType','ap.userSubscriptionDays','ap.subscriptionEndDate','ap.admin_unlock_access','ap.user_weekly_wallpaper_change_access')
				...
				if(!empty($isUsrExist)){
					...
					if(!empty($wallpaperSelected)){
						...
						if($currentDateStr <= $renewDateStr){ #die('iff');//true case
							if($wallpaperSelected->selWallpaperId != ""){
	...
	public function selectedWallpaperForAndroidUsers(Request $request){ #die('---');
		...
		if($deviceToken != ""){
			...
			if(!empty($wallpaperExist)){
				...
				//check user email exist
				$isUsrExist = DB::table('appusers as ap')
				->select('ap.id','ap.subscriptionStartDate','ap.admin_unlock_access','ap.user_weekly_wallpaper_change_access')
				->where('ap.userEmail', '=', $input["userEmail"])
				->first();
				#echo "<pre>isUsrExist==";print_r($isUsrExist);#die;
				if(!empty($isUsrExist)){
				...
				if(!empty($isUsrExist)){
					...
					if(!empty($wallpaperSelected)){
						...
						if($currentDateStr <= $renewDateStr){ //true case
							#die('ifff');
							//check if user selected wallpaper in any device under same subscription
							if($this->isSelWallInAnyDeviceInSameSubs($isUsrExist->id)){ 
								//true case(already selected wallpaper in any device for same subscription)
								$finalArr['userCanChangeWallpaper'] = "No";
							} else {
								//false case(not selected wallpaper in any device for same subscription)
								$finalArr['userCanChangeWallpaper'] = "Yes";
							}
						} else { //expired case
							#die('elseee');
