...
class MonthlyNotify extends Command
{
     ...
     * @var string
     */
    protected $signature = 'monthly:notify';
     ...
     * @var string
     */
    protected $description = 'Cron For notify for users which monthly period end before 24 hrs.';
    ...
    public function handle()
    {
		...
		where au.userSubscriptionType=2 and au.monthly_notification_sent=0 
		order by au.id asc";
		...
		if (count($subsUserResult)> 0) {
			foreach($subsUserResult as $kk=>$row) {
				...
				$next_month_subs_valid_upto = date('Y-m-d', strtotime("+1 months", strtotime($subscriptionDate )));
				$next_month_subs_intimate_date = date('Y-m-d', strtotime("-1 day", strtotime($next_month_subs_valid_upto )));
				$next_month_subs_intimate_strtotime = strtotime($next_month_subs_intimate_date);
				$today_date_strtotime = strtotime(date('Y-m-d'));
				
				#echo $next_month_subs_intimate_strtotime."==".$today_date_strtotime;#die;
				if($next_month_subs_intimate_strtotime == $today_date_strtotime){ #die('ifff');
