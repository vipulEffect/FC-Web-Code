...
class DailyQuote11 extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'quote11:daily11';
     ...
     * @var string
     */
    protected $description = 'Cron For Weekly wallpaper change at Monday at 6:30AM weekly.';
	...
	public function getFuturePastWallListForNextWeeks($futureListTillNow,$future_list,$past_list,$activeWallapersList,$week_num){
		...
		if($week_num == 1){
			...
			/*if( count($active_list_arrR) >= count($future_list_arr_till_now) ){
				$futureRR = array_values(array_diff($active_list_arrR,$future_list_arr_till_now));
			} else {
				$futureRR = array_values(array_diff($future_list_arr_till_now,$active_list_arrR));
			}*/
		...
		$futureRR = array_unique($futureRR);
		#echo "<pre>futureRR=";print_R($futureRR);die;
		if(!empty($futureRR) && count($futureRR)>=3){ #die('ifff');
			//Get Latest
			...
			if(!empty($future_list_arr)){
				...
				$exResult['past'] = array();
				$exResult['pastArr'] = array();
				$exResult['notActiveArr'] = array();
			}
		} else { #die('elsee');
			//Get Latest
			$exResult['future'] = implode(",",array_values($future_list_arr));
			$exResult['futureArr'] = array_values($future_list_arr);
			
			//Get past week list
			if(!empty($future_list_arr)){
				$exResult['past'] = implode(",",array_values($past_list_arr));
				$exResult['pastArr'] = array_values($past_list_arr);
				$exResult['notActiveArr'] = array();
			} else {
				$exResult['past'] = array();
				$exResult['pastArr'] = array();
				...
				if ( strtotime($currentDate) == strtotime($cronWeekDate) ) { 
					...
					if(!empty($ListArr)){
						...
						if($ins55){
							...
							DB::table('appusers')->update(['user_weekly_wallpaper_change_access' => '1', 'updated_at' => now()]);
